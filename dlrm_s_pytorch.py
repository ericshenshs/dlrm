...
class DLRM_Net(nn.Module):
    def create_mlp(self, ln, sigmoid_layer):
        # Creates a multi-layer perceptron (MLP) with the specified layer dimensions
        layers = nn.ModuleList()
        for i in range(0, ln.size - 1):
            n = ln[i]  # Input dimension
            m = ln[i + 1]  # Output dimension

            # Create linear layer
            LL = nn.Linear(int(n), int(m), bias=True)

            # Initialize weights using Xavier/Glorot initialization
            # For weights: std = sqrt(2/(fan_in + fan_out))
            mean = 0.0
            std_dev = np.sqrt(2 / (m + n))
            W = np.random.normal(mean, std_dev, size=(m, n)).astype(np.float32)
            
            # For bias: std = sqrt(1/fan_out) 
            std_dev = np.sqrt(1 / m)
            ...
            # Use sigmoid for specified layer, ReLU for others
            if i == sigmoid_layer:
                layers.append(nn.Sigmoid())
            else:
                layers.append(nn.ReLU())

        # Wrap layers in Sequential container for clean forward pass
